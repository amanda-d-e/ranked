...
const useGridDataSourceTreeDataPreProcessors = (privateApiRef, props) => {
  ...
  const createRowTreeForTreeData = React.useCallback(params => {
    ...
    if (!getGroupKey) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Data Grid: No getGroupKey method provided with the dataSource. ' + 'Server-side tree data requires a getGroupKey method to determine row hierarchy. ' + 'Add getGroupKey to your dataSource configuration.' : (0, _formatErrorMessage2.default)(133));
    }
    const getChildrenCount = props.dataSource?.getChildrenCount;
    if (!getChildrenCount) {
      throw new Error(process.env.NODE_ENV !== "production" ? 'MUI X Data Grid: No getChildrenCount method provided with the dataSource. ' + 'Server-side tree data requires a getChildrenCount method to determine expandable rows. ' + 'Add getChildrenCount to your dataSource configuration.' : (0, _formatErrorMessage2.default)(134));
    }
    ...
    const onDuplicatePath = (firstId, secondId, path) => {
      throw new Error(process.env.NODE_ENV !== "production" ? `MUI X Data Grid: The values returned by getGroupKey for sibling rows must be unique. ` + `Rows with id "${firstId}" and "${secondId}" have the same path: ${JSON.stringify(path.map(step => step.key))}. ` + 'Ensure getGroupKey returns unique values for each sibling row.' : (0, _formatErrorMessage2.default)(135, firstId, secondId, JSON.stringify(path.map(step => step.key))));
    };
