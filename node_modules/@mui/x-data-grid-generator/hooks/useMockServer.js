...
const useMockServer = (dataSetOptions, serverOptions, shouldRequestsFail, nestedPagination) => {
  ...
  const fetchRows = React.useCallback(async requestUrl => {
    ...
    if (shouldRequestsFailRef.current) {
      ...
      return new Promise((_, reject) => {
        ...
        setTimeout(() => reject(new Error(process.env.NODE_ENV !== "production" ? 'MUI X: Could not fetch the data' : (0, _formatErrorMessage2.default)(107))), delay);
      });
    }
    if (isTreeData) {
      const {
        ...
        rootRowCount,
        aggregateRow
      } = await (0, _serverUtils.processTreeDataRows)(dataRef.current?.rows ?? [], params, serverOptionsWithDefault, columnsWithDefaultColDef, nestedPagination ?? false);
      ...
      const {
        ...
        totalRowCount,
        aggregateRow
      } = await (0, _serverUtils.loadServerRows)(dataRef.current?.rows ?? [], (0, _extends2.default)({}, params, params.paginationModel), serverOptionsWithDefault, columnsWithDefaultColDef);
      getRowsResponse = (0, _extends2.default)({
        rows: returnedRows,
  ...
  const editRow = React.useCallback(async (rowId, updatedRow) => {
    return new Promise((resolve, reject) => {
      ...
      const maxDelay = serverOptions?.maxDelay ?? _serverUtils.DEFAULT_SERVER_OPTIONS.maxDelay;
      const delay = (0, _services.randomInt)(minDelay, maxDelay);
      const verbose = serverOptions?.verbose ?? true;
      // eslint-disable-next-line no-console
      const print = console.info;
      ...
      if (shouldRequestsFailRef.current) {
        setTimeout(() => reject(new Error(process.env.NODE_ENV !== "production" ? `MUI X: Could not update the row with the id ${rowId}` : (0, _formatErrorMessage2.default)(108, rowId))), delay);
        if (verbose) {
